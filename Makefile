...
install:
	...
	sed 's#__BIN_DIR__#$(BIN_DIR)#g;s#__DAEMON_PORT__#$(DAEMON_PORT)#g' extd.service > $(SYSTEMD_USER_DIR)/extd.service
	
	sed 's#__PRIVATE_KEY__#$(USER_KEY_FOLDER)/extd.key#g;s#__USER_HOME_DIR__#$(EXTD_USER_HOME)#g;s#__DAEMON_PORT__#$(DAEMON_PORT)#g' key_utils.py > $(BIN_DIR)/key_utils.py
	...
	sed 's#__DAEMON_PORT__#$(DAEMON_PORT)#g' extd.avahi.service > /etc/avahi/services/extd.service
