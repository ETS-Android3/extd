...
static void logcat_info(const char *format, ...) {
  ...
  if (!rfbEnableClientLogging) return;
...
static void log_obj_tostring(JNIEnv *env, jobject obj, int prio,
                             const char *format, ...) {
  if (!env) return;
 ...
 * through System.loadLibrary). JNI_OnLoad must return the JNI version needed by
 * the native library. We use this to wire up LibVNCClient logging to logcat.
 */
...
static rfbClient *getRfbClient(JNIEnv *env, jobject conn) {
  if (!env) {
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "getRfbClient failed due to env NULL");
    return NULL;
  }
...
static jboolean setRfbClient(JNIEnv *env, jobject conn, rfbClient *cl) {
  if (!env) {
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "setRfbClient failed due to env NULL");
    return JNI_FALSE;
  }
...
static void onFramebufferUpdateFinished(rfbClient *client) {
  if (!client) {
    __android_log_print(
        ...
        "onFramebufferUpdateFinished failed due to client NULL");
    return;
  }
  ...
  if (!env) {
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "onFramebufferUpdateFinished failed due to env NULL");
    return;
  }

  jclass cls = (*env)->GetObjectClass(env, obj);
  jmethodID mid =
      (*env)->GetMethodID(env, cls, "onFramebufferUpdateFinished", "()V");
  (*env)->CallVoidMethod(env, obj, mid);
}

static void onGotCutText(rfbClient *client, const char *text,
                         int __unused len) {
  if (!client) {
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "onGotCutText failed due to client NULL");
    return;
  }
  ...
  if (!env) {
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "onGotCutText failed due to env NULL");
    return;
  }

  jclass cls = (*env)->GetObjectClass(env, obj);
  jmethodID mid =
      (*env)->GetMethodID(env, cls, "onGotCutText", "(Ljava/lang/String;)V");
...
static char *onGetPassword(rfbClient *client) {
  if (!client) {
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "onGetPassword failed due to client NULL");
    return NULL;
  }
  ...
  if (!env) {
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "onGetPassword failed due to env NULL");
    return NULL;
  }

  jclass cls = (*env)->GetObjectClass(env, obj);
  jmethodID mid =
      (*env)->GetMethodID(env, cls, "onGetPassword", "()Ljava/lang/String;");
  jstring passwd = (*env)->CallObjectMethod(env, obj, mid);

  const char *cPasswd = (*env)->GetStringUTFChars(env, passwd, NULL);
  if (!cPasswd) {
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "onGetPassword failed due to cPasswd NULL");
    return NULL;
  }
  char *cPasswdCopy =
      strdup(cPasswd);  // this is free()'d by LibVNCClient in HandleVncAuth()
...
static rfbCredential *onGetCredential(rfbClient *client, int credentialType) {
  if (!client) {
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "onGetCredential failed due to client NULL");
    return NULL;
  }
  ...
  if (credentialType != rfbCredentialTypeUser) {
    ...
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "onGetCredential failed due to unsupported credential "
                        "type %d requested",
                        credentialType);
    return NULL;
  }
  ...
  if (!env) {
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "onGetCredential failed due to env NULL");
    return NULL;
  }
  ...
  jmethodID mid = (*env)->GetMethodID(
      env, cls, "onGetUserCredential",
      "()Lcom/coboltforge/dontmind/multivnc/VNCConn$UserCredential;");
  ...
  jfieldID usernameField =
      (*env)->GetFieldID(env, jCredentialCls, "username", "Ljava/lang/String;");
  jstring jUsername = (*env)->GetObjectField(env, jCredential, usernameField);

  jfieldID passwordField =
      (*env)->GetFieldID(env, jCredentialCls, "password", "Ljava/lang/String;");
  jstring jPassword = (*env)->GetObjectField(env, jCredential, passwordField);

  // Create native rfbCredential, this is free()'d by FreeUserCredential() in
  // LibVNCClient
  rfbCredential *credential = malloc(sizeof(rfbCredential));
  if (!credential) {
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "onGetCredential failed due to credential NULL");
    return NULL;
  }
  ...
  if (!cUsername) {
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "onGetCredential failed due to cUsername NULL");
    return NULL;
  }
  ...
  if (!cPassword) {
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "onGetCredential failed due to cPassword NULL");
    return NULL;
  }
...
/**
 * This basically is just a thin wrapper around the libraries built-in
 * framebuffer resizing that conveys width/height changes up to the managed
 * VNCConn.
 * @param client
 * @return
 */
static rfbBool onNewFBSize(rfbClient *client) {
  if (!client) {
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "onNewFBSize failed due to client NULL");
    return FALSE;
  }
  ...
  if (!env) {
    __android_log_print(ANDROID_LOG_ERROR, TAG,
                        "onNewFBSize failed due to env NULL");
    return FALSE;
  }

  jclass cls = (*env)->GetObjectClass(env, obj);
  jmethodID mid =
      (*env)->GetMethodID(env, cls, "onNewFramebufferSize", "(II)V");
...
static jboolean setupClient(JNIEnv *env, jobject obj, jint bytesPerPixel) {
  ...
  /*
   * We only allow 24 and 15 bit colour, as the GL canvas is not able to digest
   * any other format _directly_, without converting the whole client
   * framebuffer byte-per-byte.
   */
  switch (bytesPerPixel) {
    case 2:
      // 15-bit colour
      cl = rfbGetClient(5, 3, 2);
      // the GL canvas is using GL_UNSIGNED_SHORT_5_5_5_1 for 15-bit colour
      // depth
  ...
  if (!cl) {
    log_obj_tostring(env, obj, ANDROID_LOG_ERROR,
                     "setupClient() failed due to client NULL");
    return JNI_FALSE;
  }
...
Java_pjwstk_s18749_extd_multivnc_VNCConn_rfbShutdown(JNIEnv *env, jobject obj) {
  rfbClient *cl = getRfbClient(env, obj);
  if (cl) {
    log_obj_tostring(env, obj, ANDROID_LOG_INFO,
                     "rfbShutdown() closing connection");
    close(cl->sock);
...
JNIEXPORT jboolean JNICALL Java_pjwstk_s18749_extd_multivnc_VNCConn_rfbInit(
    JNIEnv *env, jobject obj, jstring host, jint port, jint repeaterId,
    jint bytesPerPixel) {
  log_obj_tostring(env, obj, ANDROID_LOG_INFO, "rfbInit()");

  if (!getRfbClient(env, obj)) setupClient(env, obj, bytesPerPixel);

  rfbClient *cl = getRfbClient(env, obj);

  if (!cl) {
    log_obj_tostring(env, obj, ANDROID_LOG_ERROR,
                     "rfbInit() failed due to client NULL");
    return JNI_FALSE;
  }
  ...
  log_obj_tostring(
      env, obj, ANDROID_LOG_INFO,
      "rfbInit() about to connect to '%s', port %d, repeaterId %d\n",
      cl->serverHost, cl->serverPort, cl->destPort);

  /*
   * Save pointers to the managed VNCConn and env in the rfbClient for use in
   * the onXYZ callbacks. In addition to rfbProcessServerMessage(), we have to
   * do this are as some callbacks (namely related to authentication) are called
   * before rfbProcessServerMessage().
   */
  ...
  if (!cl->frameBuffer) {
    log_obj_tostring(env, obj, ANDROID_LOG_ERROR,
                     "rfbInit() failed due to framebuffer NULL");
    Java_pjwstk_s18749_extd_multivnc_VNCConn_rfbShutdown(env, obj);
    return JNI_FALSE;
  }
...
Java_pjwstk_s18749_extd_multivnc_VNCConn_rfbProcessServerMessage(JNIEnv *env,
                                                                 jobject obj) {
  rfbClient *cl = getRfbClient(env, obj);

  if (!cl) {
    log_obj_tostring(env, obj, ANDROID_LOG_ERROR,
                     "rfbProcessServerMessage() failed due to client NULL");
    return JNI_FALSE;
  }

  /*
   * Save pointers to the managed VNCConn and env in the rfbClient for use in
   * the onXYZ callbacks. We do this each time here and not in setupClient()
   * because the managed objects get moved around by the VM.
   */
  rfbClientSetClientData(cl, VNCCONN_OBJ_ID, obj);
  rfbClientSetClientData(cl, VNCCONN_ENV_ID, env);
  ...
  if (!rfbProcessServerMessage(cl, 500)) {
    if (errno == EINTR) return JNI_TRUE;

    log_obj_tostring(env, obj, ANDROID_LOG_ERROR,
                     "rfbProcessServerMessage() failed");
    return JNI_FALSE;
  }
  return JNI_TRUE;
}
...
JNIEXPORT jboolean JNICALL
Java_pjwstk_s18749_extd_multivnc_VNCConn_rfbSetFramebufferUpdatesEnabled(
    JNIEnv *env, jobject obj, jboolean enable) {
  rfbClient *cl = getRfbClient(env, obj);
  if (cl) {
    log_obj_tostring(env, obj, ANDROID_LOG_INFO,
                     "rfbSetFramebufferUpdatesEnabled() %d", enable);
    if (enable) {
      // set
      cl->supportedMessages
          .client2server[((rfbFramebufferUpdateRequest & 0xFF) / 8)] |=
...
Java_pjwstk_s18749_extd_multivnc_VNCConn_rfbGetDesktopName(JNIEnv *env,
                                                           jobject obj) {
  rfbClient *cl = getRfbClient(env, obj);
  if (cl)
...
Java_pjwstk_s18749_extd_multivnc_VNCConn_rfbGetFramebufferWidth(JNIEnv *env,
                                                                jobject obj) {
  rfbClient *cl = getRfbClient(env, obj);
  if (cl)
    return cl->width;
...
JNIEXPORT jint JNICALL
Java_pjwstk_s18749_extd_multivnc_VNCConn_rfbGetFramebufferHeight(JNIEnv *env,
                                                                 jobject obj) {
  rfbClient *cl = getRfbClient(env, obj);
  if (cl)
    return cl->height;
...
JNIEXPORT jboolean JNICALL
Java_pjwstk_s18749_extd_multivnc_VNCConn_rfbSendKeyEvent(JNIEnv *env,
                                                         jobject obj,
                                                         jlong keysym,
                                                         jboolean down) {
  rfbClient *cl = getRfbClient(env, obj);
...
JNIEXPORT jboolean JNICALL
Java_pjwstk_s18749_extd_multivnc_VNCConn_rfbSendPointerEvent(JNIEnv *env,
                                                             jobject obj,
                                                             jint x, jint y,
                                                             jint buttonMask) {
  rfbClient *cl = getRfbClient(env, obj);
...
JNIEXPORT jboolean JNICALL
Java_pjwstk_s18749_extd_multivnc_VNCConn_rfbSendClientCutText(JNIEnv *env,
                                                              jobject obj,
                                                              jstring text) {
  rfbClient *cl = getRfbClient(env, obj);
  if (cl) {
