...
public class KeyUtils {
    ...
    public static boolean isEncrypted(String sshPrivKey) {
        KeyPair kp = KeyUtils.decryptAndRecoverKeyPair(sshPrivKey, "");
        if (kp == null)
            return true;
        return false;
    }
    ...
    public static KeyPair decryptAndRecoverKeyPair(String sshPrivKey, String passphrase) {
        ...
        try {
            if (passphrase.length() != 0) {
                ...
                byte[] decrypted = KeyUtils.decrypt(android.util.Base64.decode(sshPrivKey,
                        android.util.Base64.DEFAULT), passphrase);
                kp = KeyUtils.recoverKeyPair(decrypted);
            } else {
                Log.i(TAG, "Passphrase empty, recovering directly.");
                // There was no passphrase entered, so key is probably not encrypted.
                kp = KeyUtils.recoverKeyPair(android.util.Base64.decode(sshPrivKey,
    ...
    public static String getPubkeyString(PublicKey pubkey, String nickname) {
        try {
            return convertToOpenSSHFormat(pubkey, nickname);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public static String getPrivkeyString(PublicKey pubkey, byte[] privkey, String passphrase) {
        String data = null;

        try {
            PrivateKey pk = null;
            if (passphrase == null)
                pk = decodePrivate(privkey, "RSA");
            else
                pk = decodePrivate(privkey, "RSA", passphrase);
