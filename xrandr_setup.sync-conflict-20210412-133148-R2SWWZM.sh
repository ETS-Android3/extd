...
[ "$direction" == "left-of" ] && clip="${w}x${h}-$((w*4))+0"
[ "$direction" == "above" ] && clip="${w}x${h}+0-$((h*4))"
[ "$direction" == "below" ] && clip="${w}x${h}+0+${h}"
[ "$direction" == "right-of" ] && clip="${w}x${h}+${w}+0"
...
modeline="$(cvt "$w" "$h" "$r" | sed -n 's/.*Modeline "\([^" ]\+\)"  \(.*\)/\2/p')"
name="newmode"
...
x11vnc -ssl SAVE -avahi -clip "$clip" -noxinerama -noxrandr -nevershared -unixpw -multiptr -xkb -rfbport 5905 -bg
